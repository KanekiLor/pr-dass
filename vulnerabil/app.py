...
# vulnerabilitate 1 : http only
app.config["SESSION_COOKIE_HTTPONLY"] = False 
app.config["SESSION_COOKIE_SECURE"] = False # vulnerabilitate 2 : cookies insecure
app.config["SESSION_COOKIE_SAMESITE"] = None
app.config["PERMANENT_SESSION_LIFETIME"] = 2592000 # vulnerabilitate 3 : sesiunea de 30 de zile
...
def login():
    if request.method == "POST":
        ...
        db = get_db()
        query = f"SELECT * FROM users WHERE email = '{email}' AND password_hash = '{password}'"   # vulnerabilitate 4 : SQL injection
...
def forgot_password():
    if request.method == "POST":
        ...
        reset_token = str(user["id"])  # token predictibil, bazat pe ID-ul utilizatorului
        ...
        flash(f"Link de resetare: <a href='{reset_link}'>{reset_link}</a> (DEV MODE)")
        print(f"Token de resetare pentru {email}: {reset_token}")  
        return render_template("forgot_password.html")
